...
node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" verify commits $COMMIT_HASHES)
fi
