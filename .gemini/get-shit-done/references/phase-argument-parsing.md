...
```bash
PHASE_INFO=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
