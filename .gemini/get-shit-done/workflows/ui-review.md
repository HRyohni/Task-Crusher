...
```bash
UI_AUDITOR_MODEL=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read M:/programs/organizationTool/.gemini/agents/gsd-ui-auditor.md for instructions.
...
```bash
node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
