...
```bash
INIT=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
