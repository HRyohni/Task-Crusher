...
- **Global scope**: `M:/programs/organizationTool/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `M:/programs/organizationTool/.gemini/notes/`)
...
2. Glob `M:/programs/organizationTool/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (M:/programs/organizationTool/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `M:/programs/organizationTool/.gemini/notes/` — works in any directory
