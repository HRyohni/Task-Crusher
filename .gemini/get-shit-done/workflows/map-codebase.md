...
```bash
INIT=$(node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" init map-codebase)
...
```bash
node "M:/programs/organizationTool/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
